...
int main(void)
{
  ...
  while (1)
  {
    ...
    sscanf(request_buffer, "GET %*s HTTP/1.1"); // Ler a linha de solicitação GET
    ...
    if (strstr(request_buffer, "favicon.ico") != NULL)
    {
      ...
      {
        // Se não conseguir abrir o arquivo, envie uma resposta de erro 404
    ...
    {
      FILE *css_data = fopen("styles.css", "r");
      if (css_data == NULL)
      {
        char error_response[] = "HTTP/1.1 404 Not Found\r\n\r\n";
        sendall(client_socket, error_response, strlen(error_response));
    ...
    FILE *js_data = fopen("script.js", "r");
    if (js_data == NULL) {
        char error_response[] = "HTTP/1.1 404 Not Found\r\n\r\n";
        sendall(client_socket, error_response, strlen(error_response));
        close(client_socket);
